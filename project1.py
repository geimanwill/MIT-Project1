...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    return max(0, 1 - label * (np.dot(feature_vector.T, theta.T) + theta_0))
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    average_hinge = total / m
    return average_hinge
...
def perceptron_single_step_update(
    ...
    completed.
    """
    if label * (np.dot(feature_vector.T, current_theta.T) + current_theta_0) <= 0:
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            ...
            sum_theta += theta
            sum_theta0 += theta_0
    return (sum_theta / (feature_matrix.shape[0]*T), sum_theta0 / (feature_matrix.shape[0]*T))
...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    vector = np.dot(feature_matrix, theta) + theta_0
    vector[vector > 0] = 1
    vector[vector <= 0] = -1
    return vector
...
def classifier_accuracy(
    ...
    test_predictions = classify(val_feature_matrix, train_theta, train_theta_0)
    validation_accuracy = accuracy(test_predictions, val_labels)
